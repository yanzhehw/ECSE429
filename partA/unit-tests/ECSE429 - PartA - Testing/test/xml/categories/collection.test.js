...
describe("XML /categories", function () {
  ...
  describe("POST", function () {
    it("should create a category from a valid XML payload", async function () {
      ...
      expect(text).to.include("XML Category");
    });

    it("should reject XML payload without a title", async function () {
      const body = wrapXml("category", { description: "Missing title" });
      const res = await xmlPost(ENDPOINT, body);
    ...
    it("should reject malformed XML payload", async function () {
      const body = "<category><title>Broken<title></category>";
      const res = await xmlPost(ENDPOINT, body);
