...
describe("XML /projects/:id", function () {
  ...
  describe("POST (amend)", function () {
    it("should update fields via XML payload", async function () {
      const body = wrapXml("project", {
        active: "true",
        description: "Amended by XML",
      });
      const res = await xmlPost(`${ENDPOINT}/${EXISTING_ID}`, body);

      expect(res.status).to.equal(200);
      const text = typeof res.data === "string" ? res.data : JSON.stringify(res.data);
      expect(text).to.include("Amended by XML");
    });

    it("should return 404 for a nonexistent project", async function () {
      const body = wrapXml("project", { description: "Nope" });
  ...
  describe("PUT (replace)", function () {
    it("should replace project fields via XML payload", async function () {
      const body = wrapXml("project", {
        title: "XML Replaced",
        active: "true",
    ...
    it("should return 404 for a nonexistent project", async function () {
      const body = wrapXml("project", { title: "Ghost" });
  ...
  describe("DELETE", function () {
    it("should delete a project via XML content type", async function () {
