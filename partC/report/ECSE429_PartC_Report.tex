...
% TODO: Your team members responsible for performance testing should fill this in.
% Describe the technology stack, how measurements were taken, and the experimental setup.

\textcolor{red}{\textbf{[TO BE COMPLETED BY PERFORMANCE TESTING TEAM MEMBERS]}}

\bigskip
\noindent Describe:
\begin{itemize}
    \item The programming language and frameworks used (e.g.\ JavaScript, Mocha, Chai).
    \item How transaction time was measured (e.g.\ \texttt{perf\_hooks}).
    \item How CPU usage and free memory were tracked (e.g.\ \texttt{node-os-utils}, Activity Monitor).
    \item The sequence of object counts used for experiments (e.g.\ 1, 5, 10, 50, 100, \ldots, 10000).
    \item How the server was started and stopped between experiments.
\end{itemize}
...
\bigskip
\noindent Include experiments for each entity type (todos, projects, categories) with charts showing:
\begin{itemize}
    \item Transaction time (ms) vs.\ number of objects (for add, update, delete).
    \item CPU usage (\%) vs.\ number of objects.
    \item Free available memory (MB) vs.\ number of objects.
\end{itemize}

% Placeholder for charts -- replace with actual figure paths
% Example:
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{figures/todo_transaction_time.png}
%     \caption{Todo transaction time vs.\ number of objects.}
%     \label{fig:todo-txn}
% \end{figure}
...
\bigskip
\noindent Summarise observed performance risks and recommend code changes based on the dynamic analysis results.
